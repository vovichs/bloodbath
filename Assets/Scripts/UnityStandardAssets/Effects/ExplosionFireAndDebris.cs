		...
		private IEnumerator Start()
		{
			...
			foreach (Collider collider in array)
			{
				if (numFires > 0)
				{
					...
					if (collider.Raycast(ray, out RaycastHit hitInfo, num))
					{
			...
			{
				if (Physics.Raycast(new Ray(base.transform.position + Vector3.up, UnityEngine.Random.onUnitSphere), out RaycastHit hitInfo2, num2))
				{
